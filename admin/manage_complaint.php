...
<div class="container-fluid">
	<div class="col-lg-12">
		...
		<large><b>Image/Attachment</b></large>
		<?php if ($image_type): ?>
			<img src="data:<?php echo $image_type ?>;base64,<?php echo base64_encode($image_data) ?>" alt="Attachment"
				style="max-width: 100%; max-height: 400px;">
		<?php else: ?>
			<p>No image/attachment available</p>
		<?php endif; ?>
		...
		<form id="manage-complaints">
				...
				<div class="form-group">
					...
					<select name="responder_id" id="" class="custom-select input-sm select2">
						...
						while ($row = $complaints->fetch_assoc()):
							?>
							<option value="<?php echo $row['id'] ?>" <?php echo isset($ca['responder_id']) && $ca['responder_id'] == 1 ? 'selected' : '' ?>><?php echo $row['sname'] . ' - ' . $row['name'] ?>
							</option>
						<?php endwhile; ?>
					</select>
				</div>
			...
			<div class="action_made" style="display:none">
				<div class="form-group">
					...
					<textarea name="remarks" id="remarks" cols="30" rows="4"
						class="form-control"><?php echo isset($ca['remarks']) ? $ca['remarks'] : '' ?></textarea>
				</div>
			</div>
		</form>
	</div>
